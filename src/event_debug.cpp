  ...
  if (raw_event->event_vector_.size() < 5) {
    ...
    for (auto cluster : frame->cluster_result_neg_) {
      cv::Vec3b color{static_cast<unsigned char>(rng.uniform(0, 255)),
                      ...
                      static_cast<unsigned char>(rng.uniform(0, 255))};
      for (auto point : cluster) {
        debug_image_cluster_neg.at<cv::Vec3b>(point->event_.y_,
...
int main(int argc, char** argv) {
  ...
  intrinsic_mat << 551.0904959933358, 0, 312.5664494105658, 0,
      551.1346807591603, 236.6593903974883, 0, 0, 1;
  ...
  while (ros::ok()) {
    imu_align_unit.CheckData(data_subscriber);
